...
void set_nonblocking(int fd){
    ...
    flag |= O_NONBLOCK;
    fcntl(fd, F_SETFL, flag);       //设置回去
}

// 添加需要监听的文件描述符到epoll中
// one_shot是是否注册为 EPOLLONESHOT事件
// et是是否设置为ET模式（边沿触发模式），默认为水平触发
void addfd(int epoll_fd, int fd, bool one_shot, bool et){
    ...
    epoll_ctl(epoll_fd, EPOLL_CTL_ADD, fd, &event); //加入epoll
...
bool http_conn::write(){
    ...
    while(1) {
        // 分散写   m_write_buf[]（写缓冲区的内容） + m_file_address（客户请求的目标文件被mmap到内存中的起始位置）
        temp = writev(m_sock_fd, m_iv, m_iv_count); //写出去
        if ( temp <= -1 ) {
